...
def verify_type_compatibility(arg: Arg, field_name: str, annotation: TypeView):
    ...
    if annotation.is_union:
        all_same_arity = {
            ta.is_subtype_of((list, tuple, set))
    ...
    if annotation.is_subtype_of((list, tuple, set)):
...
def infer_long(
    ...
    if not long:
        ...
        if not annotation.is_subtype_of(bool):
            return False
...
def infer_choices(arg: Arg, annotation: TypeView) -> list[str] | None:
    if arg.choices is None:
        choices = None
        if annotation.is_subtype_of(enum.Enum):
            choices = [v.value for v in annotation.annotation]

        if annotation.is_subtype_of((tuple, list, set)):
...
def infer_action(
    ...
    if annotation.is_subtype_of((list, set)):
...
def infer_num_args(
    ...
    is_positional = not arg.short and not long
    if annotation.is_subtype_of((list, set)) and is_positional:
        return -1
